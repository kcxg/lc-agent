...
部分。因此缓存命中和写缓存部分都按各自价格**替代**输入全价，绝不能全价之外再加一份：

```python
cost = ((input_t - cache_r - cache_w) / 1_000_000) * p_in  # 未走缓存的净输入
     + (cache_r / 1_000_000) * p_cr                       # 命中缓存，替代输入价
     + (cache_w / 1_000_000) * p_cw                       # 写缓存，替代输入价
...
- `model_id`：**自己命名的、全局唯一**，标识 / 统计 / 前端显示都用它。
- `raw_model_id`：**供应商提供的原始模型名**。定价兜底 + 统计归并用。
- 接 LiteLLM 这类反代时两者一样；亲自直连多家供应商、且各家的同一个
  底层模型想分开统计时，`model_id` 可以设成不一样的。
...
```jsonc
{
  ...
  "raw_model_id": "gpt-5.4",     // 供应商提供的原始模型名 —— 定价兜底 + 统计归并用
