...
- [x] 3.3 Add frontend contract/build coverage for login routing and auth-aware API behavior.
- [x] 3.4 Run the relevant backend and frontend verification commands and record results.
...
- `uv run --extra dev ruff check lc_agent tests`: passed, all checks passed.
- `frontend/ npm run test:auth`: passed, `认证前端契约测试通过`.
- `frontend/ npm run build`: passed after restoring frontend dependencies with `npm ci`; Vite reported third-party annotation and chunk-size warnings.
